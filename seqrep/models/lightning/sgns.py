...
class SGNS(LightningModule):
    ...
    def __init__(self, num_embeddings, embedding_dim=300, weights=None, num_negative_samples=5, nn_embedding_kwargs=dict(), learning_rate=0.003):
        super().__init__()
        ...
        # embeddings lookups
        self.embeddings = nn.Embedding(num_embeddings=self.num_embeddings, embedding_dim=self.embedding_dim, **nn_embedding_kwargs)
        self.target_embeddings = nn.Embedding(num_embeddings=self.num_embeddings, embedding_dim=self.embedding_dim, **nn_embedding_kwargs)
    ...
    def forward(self, anchors, target):
        ...
        negatives = torch.multinomial(weights, batch_size * self.num_negative_samples, replacement=True)
